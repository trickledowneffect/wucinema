...
export function useJellyfinShows(options?: { filters?: string; sortBy?: string; sortOrder?: 'Ascending' | 'Descending' }) {
  const { session } = useAuth();
  const [shows, setShows] = useState<WuCinemaMediaItem[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchShows() {
      ...
      const cacheKey = `shows_${options?.filters || 'all'}_${options?.sortBy || 'DateCreated'}_${options?.sortOrder || 'Descending'}`;
